...
function ListItem({ id, name, text, type }) {
  const fetchData = useFetch();
  const dispatch = useDispatch();

  const deleteItem = () => {
    fetchData(`http://localhost:3000/heroes/${id}`, { method: 'DELETE' }).then(() => dispatch(actions.heroDeleted(id)));
  };

  return (
    <li className={['list-item', type ? `list-item_${type}` : ''].join(' ').trim()}>
